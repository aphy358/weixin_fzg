        ...
        <div v-for="(m, i) in showMonths" :key="i" class="calendar-body">
          <table>
            <thead>
              <tr>
                <th colspan="7">
                  <h5 id="first-month">{{ m.year }}年{{ m.month }}月</h5>
                </th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(week, j) in m.days.length / 7" :key="j">
                <td
                  v-for="(day, k) in m.days.slice(j * 7, (j + 1) * 7)"
                  :key="k"
                  :class="{
                ...
                >
                  <p
                    :class="{'small-text': day.festText || day.today, 'circle': checkDayStr(day)}"
                  >{{ day.festText || day.today || day.day }}</p>
                  <span
                    v-if="checkDayStr(day)"
                    :class="{
