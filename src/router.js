...
import wechatAuthPlugin from '@/components/wechatAuthPlugin'
...
// 微信授权插件初始化
Vue.use(wechatAuthPlugin, {
  router, // 路由实例对象
  appid: 'wx41041c8613e4b4b0', // 您的微信appid
  ...
  redirectUri: 'http://sz.jltour.com', //微信回调地址
  getCodeCallback (next, code) {
    ...
    axios.get('/autoLoginWx.do', {//code  ---  openid ---  user    // 通过code值换取access_token后端接口地址
      ...
      if (result) {
        next('', data.user); // 获取access_toeken成功
      } else {
